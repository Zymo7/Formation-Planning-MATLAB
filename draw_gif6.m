...
for n = 1 : num
    ...
    l5 = line([z{5}(i,2),z{6}(i,2)],[z{5}(i,1),z{6}(i,1)],'color','k');
    l6 = line([z{6}(i,2),z{1}(i,2)],[z{6}(i,1),z{1}(i,1)],'color','k');
for i = 2 : H
    delete(h1)
    delete(h2)
    delete(l1)
    ...
    if pic_num == 1
    imwrite(I,map,'test_case6.gif','gif','Loopcount',inf,'DelayTime',0.2);

    else
    imwrite(I,map,'test_case6.gif','gif','WriteMode','append','DelayTime',0.2);
    end
